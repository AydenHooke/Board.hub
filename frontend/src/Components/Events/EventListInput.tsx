      ...
      <ul className='event-list-grid'>
        {events.map((event) => (
          <li key={event.eventId} className='event-tile'>
            <h3 className='event-title' style={{cursor:"pointer"}}>Title: <Link to={`/event/${event.eventId}`}>{event.title}</Link></h3>
            ...
            <div className="event-content">
              <p data-testid="event-title">By:&nbsp;
                <Link to={`/account/${event.accountId}`}>{event.username}</Link>
              </p>
