              ...
              myMeetings.map((meeting) => (
                <li key={meeting.eventId} className='event-tile'>
                  ...
                  <div className="event-content">
                    ...
                    <button onClick={() => navigate(`/event/${meeting.eventId}`)}>View details</button>
                    {contextId && (<button onClick={() => handleLeaveEvent(meeting, "MEETING")}>{meeting.accountId === Number(contextId) ? "Delete meetup" : "Leave meetup"}</button>)}
                  </div>
                </li>
              ))
          }
